module Day9 exposing (Model, Msg, init, initAction, name, update, view)
            ...
            case step of
                Ok nextVm ->
                    case nextVm of
                        ...
                        Intcode.WaitingForInput _ _ ->
                            ( InProgressB (Intcode.giveInput (BigInt.fromInt 2) nextVm) (">> 2" :: console), False )
...
initAction =
    Advent.loadSourceFile "Day9.elm"
...
view model =
    div []
        [ C.title name
        , C.adventOfCodeProblemLink 2019 9
        , Html.p []
            [ text "Part B of today's problem is a great showcase for my async process architecture.  It takes a decent while to run, but we can do so without hanging the browser tab!" ]
        , Html.p []
        ...
        , C.section "Part B"
            [ text "Just send in a '2' and read the output."
            , C.partBSource model
            , div [ class "flex flex-column justify-center items-center" ]
    ...
    case model.processB of
        ...
        Exec.Running state _ ->
            case state of
                ...
                InProgressB vm console ->
                    div []
                        [ Html.p [] [ text (Intcode.vmToString vm) ]
                        , Html.p [] [ viewConsole console ]
                        ]
